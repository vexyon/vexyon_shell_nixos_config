...

{
  imports =
    [ # Include the results of the hardware scan.
      ...
      # Captura de la Nintendo Switch (vídeo por mpv + audio en Streams).
      ./nintendo-switch.nix
      #IA-LOCAL
      ./local-ai.nix
    ];
  ...
  users.users."vexyon" = {
    isNormalUser = true;
    description = "vexyon";
    extraGroups = [ "networkmanager" "wheel" "video" "libvirtd" "kvm" "docker" "dialout" ];
    packages = with pkgs; [];
  };
  ...
  # ---------------------------------------------------------------------
  # VIRTUALIZACIÓN: libvirt + QEMU/KVM (stack completo — Vexyon VM manager)
  # ---------------------------------------------------------------------
  virtualisation.libvirtd = {
    ...
    qemu = {
      ...
      runAsRoot = true;
      swtpm.enable = true;                    # TPM virtual — para Windows 11
    };
  };
